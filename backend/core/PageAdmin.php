...
class PageAdmin extends Okay {
    ...
    public function fetch() {
        ...
        if($this->request->method('POST')) {
            ...
            $page->compilation_id = $this->request->post('compilation_id', 'integer');
            ...
            if(($p = $this->pages->get_page($page->url)) && $p->id!=$page->id) {
                ...
                if ($this->request->post('delete_image')) {
                    $this->image->delete_image($page->id, 'image', 'pages', $this->config->original_pages_dir, $this->config->resized_pages_dir);
                }
                // Загрузка изображения
                $image = $this->request->files('image');
                if (!empty($image['name']) && ($filename = $this->image->upload_image($image['tmp_name'], $image['name'], $this->config->original_pages_dir))) {
                    $this->image->delete_image($page->id, 'image', 'pages', $this->config->original_pages_dir, $this->config->resized_pages_dir);
                    $this->pages->update_page($page->id, array('image'=>$filename));
                }
                $page = $this->pages->get_page($page->id);
            }
        } else {
