...
class ProductsAdmin extends Okay {
    ...
    public function fetch() {
        ...
        $categories_xml = $this->categories->get_categories_xml();
        $this->design->assign('categories_xml', $categories_xml);
        ...
        // Бренды категории
        $brands_count = $this->brands->count_brands(array('category_id'=> (isset($filter['category_id']) ? : '')));
        $brands = $this->brands->get_brands(array('category_id'=> (isset($filter['category_id']) ? : ''), 'limit'=>$brands_count));
        ...
        if(!empty($_GET['filter']) && is_array($_GET['filter']) && count($_GET['filter']) > 1){
        ...
        if($this->request->method('post')) {
            ...
            if(!empty($ids)) {
                switch($this->request->post('action')) {
                    ...
                    case 'move_to_xml_category': {
                        /*Переместить в категорию_xml*/
                        $category_id = $this->request->post('target_xml_category', 'integer');
                         $this->products->update_product($ids, array('xml_category_id'=>$category_id));
                        break;
                    }
                    case 'move_to_brand': {
                        /*Переместить в бренд*/
      ...
      /*  if (!empty($sku)) {
                 foreach ($this->products->get_images_new(array('sku'=> array_keys($sku))) as $im) {
                     $images[$im->sku] = $im;
                 }
            }
            */
        ...
        if(!empty($products)) {
           ...
           /* if (!empty($images)) {
                foreach ($products as &$p) {
                    if(isset($images[$p->sku])) {
                        $p->image = $images[$p->sku];
                    }
                }
            }*/
