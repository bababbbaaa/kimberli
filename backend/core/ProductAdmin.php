    ...
    public function fetch() {
        ...
        if($this->request->method('post') && !empty($_POST)) {
            ...
            if ($this->request->post('insertions')) {
                foreach ($this->request->post('insertions') as $n => $va) {
                    foreach ($va as $i => $v) {
                        if (empty($insertions[$i])) {
                            $insertions[$i] = new stdClass;
                        }
                        $insertions[$i]->$n = $v;
                        $insertions[$i]->lang_id = $this->languages->lang_id();
                    }
                }
            }
            ...
            $xml_category = $this->request->post('xml_category', 'integer');
            ...
            elseif(empty($product_categories)) {
                ...
                if($product->id) {
                    ...
                    /*Работы с вставками товара*/
                    if (is_array($insertions)) {
                        $inserions_ids = array();
                        foreach ($insertions as $index => $insertion) {

                            if (!empty($insertion->id)) {
                                $this->insertions->update_insertion($insertion->id, $insertion);
                            } else {
                                $insertion->product_id = $product->id;
                                $insertion->id = $this->insertions->add_insertion($insertion);

                            }

                            $insertion = $this->insertions->get_insertion($insertion->id);

                            if (!empty($insertion->id)) {
//                                $inserions[] = $insertion->id;
                                $inserions_ids[] = $insertion->id;
                            }
                        }

                        // Удалить непереданные варианты
                        $current_inserions = $this->insertions->get_insertions(array('product_id' => $product->id));
                        foreach ($current_inserions as $current_insertion) {
                            if (!in_array($current_insertion->id, $inserions_ids)) {
                                $this->insertions->delete_insertion($current_insertion->id);
                            }
                        }
                        // Отсортировать  варианты
                        asort($inserions_ids);
                        $i = 0;
                        foreach ($inserions_ids as $inserions_id) {
                            $this->insertions->update_insertion($inserions_ids[$i], array('position' => $inserions_id));
                            $i++;
                        }
                    }
                    ...
                    $this->products->update_product($product->id, array('main_category_id'=>$main_category->id, 'main_image_id'=>$main_image_id, 'xml_category_id' => $xml_category));
                    $product->xml_category_id = $xml_category;
            ...
            if($product) {
               ...
               // print_r($variants);
				...
				// Вставки товара
				$insertions = $this->insertions->get_insertions_shtr(array('shtr' => $var_ids));
        ...
        $this->design->assign('product_insertions', $insertions);
        ...
        $this->design->assign('brands', $brands);
                ...
                $xml_categories = [];
foreach ($this->categories->get_categories_xml() as $x){
    $xml_categories[$x->category_id] = $x->name;
}
        $this->design->assign('xml_categories', $xml_categories);
