...
{/if}
<style>
.okay_list .okay_list_row.okay_list_row_insertions{
	position: relative;
}
</style>
        ...
        <div class="col-lg-4 col-md-12 ">
            <div class="boxed fn_toggle_wrap min_height_230px">
                ...
                <div class="row">
                            ...
                            <select class="selectpicker  mb-1 fn_product_category fn_meta_categories" data-live-search="true">
                                ...
                                {function name=category_select level=0}
                                    {foreach $categories as $category}
                                        <option value="{$category->id}" {if $category->id == $first_category->id}selected{/if} data-category_name="{$category->name|escape}">{section sp $level}- {/section}{if !$level}+ {/if}{$category->name|escape}</option>
                    ...
                    <div class="col-lg-12 toggle_body_wrap on fn_card ">
                         <div class="heading_label">XML : {$btr->general_category|escape}</div>
                         <div>
                             <select name="xml_category" class="selectpicker mb-1{if !$xml_categories}hidden{/if} fn_meta_brand" data-live-search="true">
                            <option value="0" {if !$product->xml_category_id}selected=""{/if} data-xml_category_name="">{$btr->general_not_set|escape}</option>
                            {foreach $xml_categories as $k => $name}
                                <option value="{$k}" {if $product->xml_category_id == $k}selected=""{/if} data-xml_category_name="{$name|escape}">{$name|escape}</option>
                            {/foreach}
                        </select>
                         </div>
                    </div>
	...
	<div class="row vsravka-row">
        <div class="col-lg-12 col-md-12 ">
            <div class="boxed fn_toggle_wrap match_matchHeight_true">
                ...
                <div class="vstavkas_wrapper fn_card">
                    <div class="okay_list vstavkas_list scrollbar-vstavka">
                        <div class="okay_list_body sortable insertions_listadd">
                            {foreach $product_insertions as $insertion}
                                <div class="okay_list_body_item insertions_list_item">
                                    <div class="okay_list_row okay_list_row_insertions">
                                        <div class="okay_list_boding vstavkas_item_name">
                                            <div class="heading_label">Название</div>
                                            <input name="insertions[id][{$insertion->id|escape}]" type="hidden" value="{$insertion->id|escape}" />
                                            <input class="vstavka_input" name="insertions[name][{$insertion->id|escape}]" type="text" value="{$insertion->name|escape}" />
                                        </div>
                                        <div class="okay_list_boding vstavkas_item_name">
                                            <div class="heading_label">Форма</div>
                                            <input class="vstavka_input" name="insertions[form][{$insertion->id|escape}]" type="text" value="{$insertion->form|escape}"/>
                                        </div>
                                        <div class="okay_list_boding vstavkas_item_name">
                                            <div class="heading_label">Масса</div>
                                            <input class="vstavka_input" name="insertions[mass][{$insertion->id|escape}]" type="text" value="{$insertion->mass|escape}"/>
                                        </div>
                                        <div class="okay_list_boding vstavkas_item_name">
                                            <div class="heading_label">Количество граней</div>
                                            <input class="vstavka_input" name="insertions[count][{$insertion->id|escape}]" type="text" value="{$insertion->count|escape}"/>
                                        </div>
                                        
                                        <div class="okay_list_boding okay_list_close remove_vstavka">
                                           <button data-hint="{$btr->general_delete_product|escape}" type="button" class="btn_close fn_remove_insertion hint-bottom-right-t-info-s-small-mobile  hint-anim">
                                               {include file='svg_icon.tpl' svgId='delete'}
                                           </button>
                                        </div>
                                    </div>
                            ...
                            <div class="okay_list_body_item insertions_list_item fn_new_row_insertion">
                                <div class="okay_list_row ">
									<div class="okay_list_boding vstavkas_item_name">
                                        <div class="heading_label">Название</div>
                                        <input name="insertions[id][]" type="hidden" value="{$insertion->id|escape}" />
                                        <input class="vstavka_input" name="insertions[name][]" type="text" value="{$insertion->name|escape}" />
                                    </div>
                                    <div class="okay_list_boding vstavkas_item_name">
                                        <div class="heading_label">Форма</div>
                                        <input class="vstavka_input" name="insertions[form][]" type="text" value="{$insertion->form|escape}"/>
                                    </div>
                                    <div class="okay_list_boding vstavkas_item_name">
                                        <div class="heading_label">Масса</div>
                                        <input class="vstavka_input" name="insertions[mass][]" type="text" value="{$insertion->mass|escape}"/>
                                    </div>
                                    <div class="okay_list_boding vstavkas_item_name">
                                        <div class="heading_label">Количество граней</div>
                                        <input class="vstavka_input" name="insertions[count][]" type="text" value="{$insertion->count|escape}"/>
                                    </div>
					 ...
					 <div class="box_btn_heading mt-1">
                        <button type="button" class="btn btn_mini btn-info fn_add_insertion">
                            {include file='svg_icon.tpl' svgId='plus'}
                            <span>Добавить вставку</span>
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    $(window).on("load", function() {
        ...
        $(document).on("click",".fn_remove_insertion",function () {
            $(this).closest(".insertions_list_item ").fadeOut(200);
            $(this).closest(".insertions_list_item ").remove();
        });
        
        // Добавление вставок
        var vstavka = $(".fn_new_row_insertion").clone(false);
        $(".fn_new_row_insertion").remove();
        vstavka.find('.bootstrap-select').replaceWith(function() { return $('select', this); });
        $(document).on("click", ".fn_add_insertion", function () {
           var vstavka_clone = vstavka.clone(true);
           vstavka_clone.find("select").selectpicker();
           vstavka_clone.removeClass("hidden").removeClass("fn_new_row_insertion");
           $(".insertions_listadd").append(vstavka_clone);
            return false;
        });
