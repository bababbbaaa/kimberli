...
class Variants extends Okay {
    ...
    public function get_variants($filter = array()) {
       ...
       /* if (!empty($filter['sort'])) {
			switch ($filter['sort']) {
				case 'price': $variant_sort = 'v.price ASC'; break;
				case 'price_desc': $variant_sort = 'v.price DESC'; break;
			}
        ...
        $query = $this->db->placehold("SELECT 
                ...
                IF((v.compare_price != v.price AND v.compare_price > 0), v.proc, 0) as discount,
                v.sku,
       			v.new_sku,    
       			v.url,
       			v.size,
