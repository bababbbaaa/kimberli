{* Login page *}

{* The canonical address of the page *}
{$canonical="/user/login" scope=parent}

{* The page title *}
{$meta_title = $lang->login_title scope=parent}

<div class="page_container">

    {* The page heading *}
    <div class="wrap_blog_heading_page">
        <h1 class="heading_page">
            <span data-language="login_enter">{$lang->login_enter}</span>
        </h1>
    </div>
    
    <div class="accaunt_promo_heading">
        <span data-language="login_text">{$lang->login_text}</span>
        {* Link to registration *}
        <a href="{$lang_link}user/register" class="button" data-language="login_registration">{$lang->login_registration}</a>
    </div>
    
    <div class="block padding">
        <div class="">
            <div class="">
                {* Form error messages *}
                {if $error}
                    <div class="message_error">
                        {if $error == 'login_incorrect'}
                            <span data-language="login_error_pass">{$lang->login_error_pass}</span>
                        {elseif $error == 'user_disabled'}
                            <span data-language="login_pass_not_active">{$lang->login_pass_not_active}</span>
                        {else}
                            {$error|escape}
                        {/if}
                    </div>
                {/if}

                <form method="post" class="clearfix fn_validate_login">

                    <div class="row">
                        <div class="col-lg-6 form_group">
                            {* User's email *}
                            <input class="form_input_accaunt" type="text" name="email" value="{$email|escape}" data-language="form_email" placeholder="{$lang->form_email}*"/>
                        </div>


                        <div class="col-lg-6 form_group">
                            {* User's password *}
                            <input class="form_input_accaunt" type="password" name="password" data-notice="{$lang->form_enter_password}" value="" data-language="form_password" placeholder="{$lang->form_password}*"/>
                        </div>
                    </div>    
                    
                    <div class="boxed_button">
                        {* Submit button *}
                        <input type="submit" class="btn btn_black btn_account" name="login" data-language="login_sign_in" value="{$lang->login_sign_in}">
                    </div>

                    <div class="boxed_button">
                        {*  Remind password link *}
                        <a class="password_remind" href="{$lang_link}user/password_remind" data-language="login_remind">{$lang->login_remind}</a>
                    </div>    
                </form>
            </div>
        </div>
    </div>
</div>
