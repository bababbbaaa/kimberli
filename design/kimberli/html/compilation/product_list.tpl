...
<div class="preview fn_product">
    <div class="fn_transfer clearfix">
        ...
        {foreach $product->images as $img}
            {if $img@iteration == 2}
            {assign var="second_image" value=$img->filename|resize:350:380}
            {/if}
        {/foreach}
        <div class="product_images bg_grey ">
            ...
            <a class="preview_image" href="{if $smarty.get.module=='ComparisonView'}{$product->image->filename|resize:800:600:w}{else}{$lang_link}products/{$product->url}{/if}" {if $smarty.get.module=='ComparisonView'}data-fancybox="group" data-caption="{$product->name|escape}"{/if}>
                {if $product->image->filename}
                    <img class="fn_img preview_img" src="{$product->image->filename|resize:350:380}" data-imageoriginal="{$product->image->filename|resize:350:380}" data-imagehover="{$second_image}" alt="{$product->name|escape}" title="{$product->name|escape}"/>
                {else}
