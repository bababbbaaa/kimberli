<div class="hidden">
    ...
    <div class="hidden">
        <div id="coupon_sms_verification" style="text-align: center;">
            {* The form heading *}
            <div class="popup_heading">
            ...
            <input class="btn btn_big btn btn_black btn_cart send_code_button" style="margin-top: 20px;" disabled="true" type="submit" name="submit2" data-method="confirm" onclick="return checkValidCode('coupon_form', 'coupon_sms_verification');"  data-language="sms_code_checkout"  value="{$lang->sms_code_checkout}">

        </div>
    </div>
    <literal>
        <script>
            $('#coupon_sms_verification #sms_code_run').on('keyup', function(e) {
                let value =  $(this).val();
                if(
                    value != ''
                    && value.length == 4
                    ...
                    $('#coupon_sms_verification .send_code_button').removeAttr('disabled');
                } else {
                    $('#coupon_sms_verification .send_code_button').prop("disabled", true);
                }
            });
        </script>
    </literal>