    ...
    public function checkout_form($params) {
      //  print_r($params);
        ...
        if(!empty($module_name) && is_file("payment/$module_name/$module_name.php")) {
         ...
         /*   
            switch ($module_name){
                case 'Liqpay': $module = new $module_name(); $form =  $module->checkout_form($params['order_id']); break;
                case 'OKPay': $module = new $module_name(); $this->design->assign('forma', false); break;
                case 'IBAN': $module = new $module_name(); $this->design->assign('forma', $module->checkout_form($params['payment_method_id'])); break;
                case 'Card': $module = new $module_name(); $this->design->assign('forma', $module->checkout_form($params['payment_method_id'])); break;
                case 'PayParts':
                    $order = $this->orders->get_order((int)$params['order_id']);
                    $settings = $this->payment->get_payment_settings($order->payment_method_id);
                    //$ProductsList = $this->orders->get_purchases(array('order_id' => intval($order->id)));
                    $module = new $module_name($settings['store_id'], $settings['password']);
                   ...
                   switch ($order->payment_method_id){
                       case '19': $this->design->assign('forma',$module->pp($settings['parts_count'], $order, $params['product_list'])); break;//частями
                       case '20':  $this->design->assign('forma',$module->ip($settings['parts_count'], $order, $params['product_list'])); break;//розсрочка
                   }
                    
                    break;
              //  default: $this->design->assign('forma', $form); break;
            }*/
            if(!empty($form)) {
