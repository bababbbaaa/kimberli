...
class ProductsView extends View {
    ...
    private $other_filters = array( 'featured' );//'discounted',
    private $category;
    ...
    public function __construct() {
        ...
        foreach($this->uri_array as $k=>$v) {
            ...
            if(!$k && $brand=$this->brands->get_brand((string)$v)) {
                ...
                if (in_array($this->page->url, array('catalog','all-products', 'stock', 'discounted', 'bestsellers'))
                ...
                switch($param_name) {
                    ...
                    case 'sort': {
                        ...
                        if (!in_array($_GET['sort'], array('position', 'price', 'price_desc', 'name', 'name_desc', 'rating', 'rating_desc', 'sku', 'sku_desc'))) {
                            $this->is_wrong_params = 1;
                        }
                        break;
                    }
    ...
    private function filter_chpu_parse_url() {
        ...
        preg_match("~^$this->subdir(/?$this->lang_label)?/?(catalog|all-products|stock|brands|discounted|bestsellers)/?~", $uri['path'], $this->catalog_type);
        $this->catalog_type = $this->catalog_type[2];
        //убираем модификатор каталога
        $uri = preg_replace("~^$this->subdir(/?$this->lang_label)?/?(catalog|all-products|stock|brands|discounted|bestsellers)/?~",'',$uri['path']);
