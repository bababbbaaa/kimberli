<?php
    ...
    if (count($cart->purchases) > 0) {
        ...
        $result['cart_informer'] = $cart->total_products;//$okay->design->fetch('cart_informer.tpl');
        $result['cart_purchases'] = $okay->design->fetch('cart_purchases.tpl');
        $result['cart_popap'] = $okay->design->fetch('cart/cart_popap.tpl');
